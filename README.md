...
* **降噪处理**
	...
	![Compare between original and denoised curve](pic/denoised.png)
...
### 3. 输入模拟与伽马信号源的判定
<!--TODO-->
对于伽马暴事件，将其分为以下两种情况分别处理：
1. 有一个及以上的卫星测得明显信号；
2. 所有卫星上均未测得明显可分辨的信号

其中信号明显的标准为信号最高峰与方差的比值大于3.5

第一种情况

* **计算初始（猜测）方向** 将接收到明显信号的卫星的法向单位矢量相加作为初始方向。
* **生成模拟入射的方位** 在以初始方向为中心的半球上($\theta \in [0, \pi/2], \phi \in [0, 2\pi]$)设置模拟的入射点。设置方法为将$\theta$等分为15份，$\phi$等分为60份，共900个入射点。
* **在每个入射点模拟入射并打分** 将每个生成的模拟点作为伽马暴的入射点，计算各卫星在前三个能道理论上接收到的能量并将其归一。
取各卫星在前三个能道实际接收到的能量并归一，将各卫星各能道理论和实际的接收能量作差、平方并求和作为该入射点的得分。对所有入射点模拟完成后，将得分最低的点作为分析得到的伽玛暴入射方位。

第二种情况

* **生成模拟入射的方位** 在整个球面上设置模拟的入射点。设置方法为将$\theta$等分为30份，$\phi$等分为60份，共1800个入射点。
* **在每个入射点模拟入射并打分** 方法同上。
...
### `find_gamma.py`: 分析各卫星接收到的能谱，推测入射伽玛暴的方位
<!--TODO:?-->
此为推测入射伽玛暴方位的模块。在此程序中，首先读取各卫星接收到的能谱。根据探测到明显信号的卫星数的不同进行不同的模拟。比较模拟结果和真实结果，输出两者差距最小的位置作为推测的入射伽玛暴方位。
